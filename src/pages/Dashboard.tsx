  ...
  return (
    <div className="space-y-8">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          <h1 className="text-3xl font-bold text-neutral-900 dark:text-neutral-50 tracking-tight">Dashboard Overview</h1>
          <p className="text-neutral-500 mt-1">Status: {dateFilter === 'today' ? 'Today\'s Summary' : `From ${startDate} To ${endDate}`}</p>
        </div>
      </div>

      {/* Date-Picker & Header */}
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 bg-white dark:bg-neutral-900 p-5 rounded-2xl shadow-sm border border-neutral-100 dark:border-neutral-800">
        ...
        <div className="flex flex-wrap items-center gap-4">
          {/* Quick Filter Toggle */}
          <div className="flex bg-neutral-100 dark:bg-neutral-800 p-1.5 rounded-xl border border-neutral-200/50">
            <Button 
              variant="ghost" 
              size="sm" 
              className={`h-9 px-4 rounded-lg text-sm font-semibold transition-all ${dateFilter === 'today' ? 'bg-white dark:bg-neutral-700 shadow-sm text-teal-600' : 'text-neutral-500'}`}
            ...
            <Button 
              variant="ghost" 
              size="sm" 
              className={`h-9 px-4 rounded-lg text-sm font-semibold transition-all ${dateFilter === 'custom' ? 'bg-white dark:bg-neutral-700 shadow-sm text-teal-600' : 'text-neutral-500'}`}
          ...
          <div className={`flex flex-wrap items-center gap-2 transition-opacity ${dateFilter === 'today' ? 'opacity-50 pointer-events-none' : 'opacity-100'}`}>
            <div className="flex items-center gap-2 bg-neutral-50 dark:bg-neutral-800/50 px-3 py-2 rounded-lg border border-neutral-200 dark:border-neutral-700">
          ...
          <div className="h-8 w-px bg-neutral-200 dark:bg-neutral-800 hidden lg:block"></div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-6">
        ...
        <Card className="border-none shadow-sm bg-white dark:bg-neutral-900 border border-neutral-100">
          <CardContent className="pt-6">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-xs font-medium text-neutral-500 uppercase tracking-wider">Total In Stock</p>
                <div className="flex flex-col">
                  <h3 className="text-2xl font-bold mt-1 text-neutral-900 dark:text-neutral-50">{stats.totalProductTypes || 0} Types</h3>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <Card className="lg:col-span-1 border-none shadow-sm bg-white dark:bg-neutral-900">
        ...
        <Card className="lg:col-span-2 border-none shadow-sm bg-white dark:bg-neutral-900 border border-neutral-100">
          <CardHeader>
          ...
          <CardContent>
            <div className="grid grid-cols-2 gap-4">
              <div className="p-4 rounded-xl bg-blue-50 dark:bg-blue-900/10 border border-blue-100 dark:border-blue-800">
                ...
                <p className="text-2xl font-bold text-blue-900 dark:text-blue-50">₹{(stats.totalSales || 0).toLocaleString()}</p>
              ...
              <div className="p-4 rounded-xl bg-teal-50 dark:bg-teal-900/10 border border-teal-100 dark:border-teal-800">
                ...
                <p className="text-2xl font-bold text-teal-900 dark:text-teal-50">₹{(stats.totalCreditSales || 0).toLocaleString()}</p>
              ...
              <div className="p-4 rounded-xl bg-amber-50 dark:bg-amber-900/10 border border-amber-100 dark:border-amber-800">
                ...
                <p className="text-2xl font-bold text-amber-900 dark:text-amber-50">₹{(stats.duesPaid || 0).toLocaleString()}</p>
                <p className="text-[10px] text-amber-400 mt-1">Debt recovered in period</p>
              </div>
              <div className="p-4 rounded-xl bg-rose-50 dark:bg-rose-900/10 border border-rose-100 dark:border-rose-800 shadow-sm border-2">
                <p className="text-xs font-bold text-rose-600 uppercase mb-1">Dues Remaining</p>
                <p className="text-2xl font-bold text-rose-900 dark:text-rose-50">₹{(stats.outstandingCredit || 0).toLocaleString()}</p>
